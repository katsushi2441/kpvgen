...
def heartmula_generate_job(
    ...
    **_: Any,
    ...
    r = subprocess.run(SSH + [setup], input=lyrics.strip() + "\n", capture_output=True, text=True, timeout=60)
    if r.returncode != 0:
        raise RuntimeError(f"remote setup failed: {r.stderr.strip()[:200]}")
    steps.append(f"remote_dir:{remote_dir}")
    cmd = (f"cd {HEARTLIB} && .venv/bin/python examples/run_music_generation.py --model_path ./ckpt --version 3B "
    ...
    t0 = time.time()
    r = subprocess.run(SSH + [cmd], capture_output=True, text=True, timeout=generation_timeout)
    elapsed = round(time.time() - t0)
    if r.returncode != 0:
        tail = subprocess.run(SSH + [f"tail -c 1500 {shlex.quote(remote_dir + '/run.log')}"],
    ...
    r = subprocess.run(SCP + [f"{HOST}:{remote_dir}/output.mp3", str(dst)], capture_output=True, text=True, timeout=300)
